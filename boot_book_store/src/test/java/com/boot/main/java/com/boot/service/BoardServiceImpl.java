...
//    }
    ...
    public void delete(Long boardNo) {
        if (boardNo == null) throw new IllegalArgumentException("boardNo는 필수입니다.");

        // 첨부파일 목록 조회 (파일 삭제용)
        List<BoardAttachDTO> attaches = attachDAO.findByBoardNo(boardNo);

        // 첨부파일 DB 데이터 삭제
        attachDAO.deleteByBoardNo(boardNo);

        // 게시글 삭제
        boardDAO.delete(boardNo);

        // 실제 파일 삭제 (선택: 필요 없으면 주석 처리 가능)
        for (BoardAttachDTO att : attaches) {
            String filePath = att.getFilePath().replace("/upload", uploadDir);
